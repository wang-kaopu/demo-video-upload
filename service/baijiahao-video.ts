 ...
 */
function createHttpClient(logger: Logger, responses: SerializedAxiosResponse[]): AxiosInstance {
...
export async function resolveFirstTopic(
  ...
  return resolveTopicsInOrder(topicNames, (name) => searchTopic(cookieHeader, name, http));
}
 ...
 * 独立执行百家号视频素材准备和最终发布。
 */
export class BaijiahaoVideo implements Video<BaijiahaoPreparedContext, BaijiahaoPublishResponse> {
  private readonly http: AxiosInstance;
  private readonly logger: Logger;
  private readonly options: BaijiahaoVideoOptions;
   ...
   */
  public constructor(options: BaijiahaoVideoOptions, dependencies: VideoDependencies) {
    ...
    this.http = createHttpClient(this.logger, this.responses);
  }

  /**
   * 完成账号验证、预上传、封面、视频分片、汇总和话题准备。
   *
   * @returns 最终发布所需的完整上下文
   */
  public async prepare(): Promise<BaijiahaoPreparedContext> {
    const responseStart = this.responses.length;
    const [cookieHeader, publication, metadata, videoStats, fileMd5] = await Promise.all([
      loadCookieHeader(this.options.cookiesPath),
      readFile(this.options.textPath, "utf8").then(parsePublicationText),
      inspectMp4(this.options.videoPath),
      stat(this.options.videoPath),
      calculateFileMd5(this.options.videoPath),
    ]);
    await stat(this.options.coverPath);
    const context: BaijiahaoRunContext = {
      ...
      metadata,
      publication,
      videoName: basename(this.options.videoPath),
    };
    ...
    context.appId = await fetchAppId(cookieHeader, this.http);
    await emitLog(this.logger, { message: "[2/7] 创建视频预上传任务", type: "info" });
    const upload = await preUploadVideo(context, this.http);

    await emitLog(this.logger, { message: "[3/7] 从单一封面生成竖版和横版 JPEG，并依次上传", type: "info" });
    const covers = await generateCovers(this.options.coverPath);
    const verticalCover = await uploadCover(cookieHeader, covers.vertical, this.http);
    const horizontalCover = await uploadCover(cookieHeader, covers.horizontal, this.http);

    await emitLog(this.logger, { message: "[4/7] 上传 2 MiB 视频分片", type: "info" });
    const chunks = await uploadVideoChunks(
    ...
    await emitLog(this.logger, { message: "[5/7] 汇总视频上传信息", type: "info" });
    await completeVideoUpload(context, upload.uploadKey, chunks, this.http);

    await emitLog(this.logger, { message: "[6/7] 搜索话题并构造最终发布参数", type: "info" });
    const topic = await resolveFirstTopic(cookieHeader, publication.topicNames, this.http);
    ...
    await emitLog(this.logger, { message: `========== PUBLISH PAYLOAD ==========\n${JSON.stringify(payload, null, 2)}`, type: "info" });
   ...
   * 使用准备结果执行最终发布。
   *
   * @param prepared - prepare 返回的完整发布上下文
  * @returns nid 和完整可序列化 Axios 响应
  */
  public async publish(prepared: BaijiahaoPreparedContext): Promise<BaijiahaoPublishResponse> {
    const bodyText = serializePublishPayload(prepared.payload);
    const response = await this.http.post<PublishResponse>(PUBLISH_URL, bodyText, {
      headers: {
    ...
    return { nid: String(nid), response: await serializeAxiosResponse(response) };
  }
